class ApplicationController < ActionController::Base
  ...
  def require_user
    unless current_user
      if request.get?
        redirect_to :controller => "oauth_map4d", :action => "signin", :referer => request.fullpath
      else
        head :forbidden
      end
    end
  ...
  def deny_access(_exception)
    if current_token
      ...
      respond_to do |format|
        format.html { redirect_to :controller => "oauth_map4d", :action => "signin", :referer => request.fullpath }
